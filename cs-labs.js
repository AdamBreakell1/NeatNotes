...
function validateLabs(labs = LABS) {
  ...
  labs.forEach((item) => {
    ...
    if (!["published", "review_pending", "academically_reviewed"].includes(item.reviewStatus)) errors.push(`Invalid review state: ${item.id}`);
    if (item.responseType === "choice" && (!item.options.includes(item.answer) || new Set(item.options).size !== item.options.length)) errors.push(`Invalid choices: ${item.id}`);
  });
