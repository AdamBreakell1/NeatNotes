...
const rows = [
  ["2.1.1", "model-devise", "Choose a model", "A planner finds the fewest train changes, not the shortest distance. Which graph model best serves that purpose?", ["Station-and-service pairs as nodes; staying on a service costs zero and changing service costs one", "Stations as nodes; every kilometre adds one", "Station-and-service pairs as nodes; every stop costs one regardless of changes", "Stations as nodes; every minute of travel adds one"], 0, "Track the service as well as the station so the graph can distinguish staying aboard from a change. Zero-cost travel edges and one-cost transfer edges minimise changes; distance, stops and time optimise different objectives."],
